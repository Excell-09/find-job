...
const AppProvider = ({ children }) => {
  ...
  const getJobs = async ({ page, search, searchStatus, searchType, sort }) => {
    startLoading();
  ...
  const getJobsHistory = async ({ page, search, searchStatus, searchType, sort }) => {
    startLoading();

    let url = `/jobs.hisoty?page=${page}&status=${searchStatus}&jobType=${searchType}&sort=${sort}`;
    if (search) {
      url = url + `&search=${search}`;
    }
    try {
