...
const AppProvider = ({ children }) => {
  ...
  const deleteJob = async (jobId) => {
    ...
    try {
      ...
      displayAlertClear('Job Deleted!', 'success');
      dispatch({ type: 'RE_RENDER', rerender: state.rerender });
    } catch (error) {
      if (error.response.status === 401) return;
      displayAlertClear(error.response.data.msg, 'error');
    }
    stopLoading();
  };
  ...
  const editJob = async ({ position, company, jobLocation, status, jobType }) => {
    ...
    try {
      ...
      displayAlertClear('Job Updated!', 'success');
    } catch (error) {
      if (error.response.status === 401) return;
      logoutUser();
    }
