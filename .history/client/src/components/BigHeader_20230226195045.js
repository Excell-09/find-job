...
const BigHeader = () => {
  ...
  const {user,logoutUser,idEditting} = useAppContext()
  ...
  return (
    ...
    <div className='flex items-end space-x-1 cursor-pointer' onClick={()=> navigate('/')}>
      ...
      <button className='relative z-50' type='button' >
        ...
        <div className={` w-[120px] transition-all duration-200 font-semibold cursor-pointer shadow rounded-full px-4 py-1 absolute bg-red-500 translate-y-1 text-white ${dropDown ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={()=> logoutUser()}>Log Out</div>
      </button>
    </header>
