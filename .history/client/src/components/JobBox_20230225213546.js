      ...
      <div className='mb-24 mt-5 md:my-5'>
        <div className='flex items-center justify-center md:justify-end'>
          ...
          {Array.from({ length: numOfPages }, (_, i) => i + 1).map((item, i) => {
            return (
              <div className={`${item === page && 'bg-indigo-500'} border-2 bg-indigo-300 w-9 h-9 text-white font-bold flex items-center justify-center hover`} key={i} onClick={()=> setPage(item)}>
                <p>{item}</p>
              </div>
            );
          })}
