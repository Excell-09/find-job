...
const CardJob = ({ job, ableEdit = false }) => {
  ...
  const { setIdJob, loading } = useAppContext();
      ...
      {ableEdit && (
        <div className='flex gap-3'>
          <button
            ...
            onClick={() => {
              navigate('/create');
              setIdJob({ id: job._id, job: job });
            }}>
            Edit
          </button>
          <button disabled={loading} className='button bg-red-500 hover:bg-red-400 disabled:bg-red-500 disabled:hover:bg-red-500' onClick={deleteJob(job._id)}>
            Delete
          </button>
        </div>
      )}
