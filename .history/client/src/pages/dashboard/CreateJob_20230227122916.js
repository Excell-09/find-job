...
const CreateJob = () => {
  const { isEditing, loading, jobLocation, createJob,      displayAlertClear('Job Created!', 'success');
 } = useAppContext();
  const initialstate = {
    position: '',
    company: '',
  ...
  const handleSubmit = () => {
    ...
    isEditing ? createJob({}) : createJob({ position, company, jobLocation, status, jobType });
    handleClear();
  };
  return (
    <div className='bg-white p-5 rounded-sm shadow-lg'>
      ...
      <form className='grid grid-cols-8 gap-4' onSubmit={handleSubmit}>
        ...
        <div className=' col-span-8 sm:col-span-2 flex items-end gap-1'>
          <button disabled={loading} className='shadow-md col-span-2 smallbtn py-2' type='submit'>
            {isEditing ? 'Edit' : 'Create'}
          </button>
          <button onClick={isEditing ? () => {} : handleClear} className='shadow-md py-2 smallbtn bg-red-500 hover:bg-red-400' type='button'>
